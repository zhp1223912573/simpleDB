...
//		myLogger.logger.debug("\n\n\n");
		...
		for(int i = 0; i < 100; i++) {
			...
			myLogger.logger.debug("****要插入的第"+i+"个元组为"+t+"****");
			Database.getBufferPool().insertTuple(tid, bigFile.getId(), t);
			myLogger.logger.debug("****第"+i+"个元组成功插入****\n");
...
////				Tuple next = fit.next();
////				if(!next.equals(t)){
////					myLogger.logger.info("visiting元组"+next+
////							"	pageId："+next.getRecordId().getPageId()+
////							"	tupleNo:"+next.getRecordId().getTupleNumber());
////					myLogger.logger.info("**键值相同，其余不同\n");
////				}
//				if(fit1.next().equals(t)) {
//					found1 = true;
//					myLogger.logger.error("**找到第"+i+"个插入的元组****\n");
//					break;
//				}
//			}
//			int stop = -1;
//			if(!found1) {
//				myLogger.logger.error("**找不到第"+i+"个插入的元组****\n");
//				stop=i;
//			}
//			fit1.close();
//
...
//				}
				...
				if(fit.next().equals(t)) {
					found = true;
					myLogger.logger.info("**找到第"+i+"个插入的元组****\n");
					break;
				}
			}
			fit.close();
...
//			myLogger.logger.debug("****第"+i+"个元组成功插入****\n");
		...
		while(fit.hasNext()) {
			Tuple tup = fit.next();
			if(prev != null) {
//				if (tup.getField(0).compare(Op.LESS_THAN, prev.getField(0))) {
//
//					myLogger.logger.error("排序不正确的元组为：pre-"+prev+" after-"+tup);
//					myLogger.logger.error("当前遍历到了第"+count+"个元组");
//					myLogger.logger.error("************************");
//
//					prev = tup;
//					count++;
