...
public class BTreeLeafPage extends BTreePage {
	...
	public void deleteTuple(Tuple t) throws DbException {
		...
		myLogger.logger.debug("正在删除元组"+t);
	...
	public void insertTuple(Tuple t) throws DbException {
		...
		t.setRecordId(rid);
		tuples[goodSlot] = t;
	myLogger.logger.debug("正在插入元组"+tuples[goodSlot]);
//		if(t.getField(0).equals(t.getField(1))){
//			myLogger.logger.debug("正在插入元组"+tuples[goodSlot]);
////			myLogger.logger.debug("BTreeLeafPage.insertTuple: new tuple, tableId = "
////					+tuples[goodSlot].getRecordId().getPageId().getTableId()+
////					" pageId =" +tuples[goodSlot].getRecordId().getPageId().getPageNumber()+
////					" slotId = "+goodSlot);
////			myLogger.logger.error("插入元组的后一个元组为"+tuples[goodSlot+1]);
////			if(goodSlot<tuples.length-1&&tuples[goodSlot+1]!=null&&((IntField)tuples[goodSlot+1].getField(0)).getValue()<((IntField)t.getField(0)).getValue()){
////				myLogger.logger.error("插入当前元组后出现顺序错误！！");
////				myLogger.logger.error("插入的测试元组为"+t);
////				myLogger.logger.error("插入元组的后一个元组为"+tuples[goodSlot+1]);
////				myLogger.logger.error("BTreeLeafPage.insertTuple: new tuple, tableId = "
////								+pid.getTableId()+" pageId ="
////								+pid.getPageNumber()+"slotId = "+goodSlot);
////
////				tuples[goodSlot] =  tuples[goodSlot+1];
////				tuples[goodSlot+1] = t;
////				myLogger.logger.error("将出现错误排序的元组进行调换");
////				return ;
////			}
//		}
...
class BTreeLeafPageIterator implements Iterator<Tuple> {
	...
	public boolean hasNext() {
		...
		try {
			while (true) {
				//myLogger.logger.info("在BTreeLeafPage内查找插入元素，当前遍历到"+curTuple);
				nextToReturn = p.getTuple(curTuple++);
				if(nextToReturn != null)
					return true;
			}
		} catch(NoSuchElementException e) {
			//myLogger.logger.info("在BTreeLeafPage内不存在插入元素，当前遍历到"+curTuple);
			return false;
		}
	}
